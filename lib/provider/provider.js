...
const sign = (shaSumsFile, targetDir, gpgKey, gpgPassphrase) => new Promise((resolve, reject) => {
  let keyOption = '';
  if (gpgKey) {
    keyOption = `--default-key ${gpgKey} `;
  }
  let passphraseOption = '';
  if (gpgPassphrase) {
    passphraseOption = `--pinentry-mode=loopback --passphrase ${gpgPassphrase}`;
  }
  exec(`gpg ${passphraseOption} --detach-sign ${keyOption} --yes ${shaSumsFile}`, { cwd: targetDir }, (err) => {
    if (err) {
      return reject(err);
    }
        ...
        (error) => {
          ...
          if (error.code === 'ECONNREFUSED') {
            ...
            if (response && response.body) {
              ...
              error.message = errors.map((msg) => `${msg}`)
                .join('\n');
            }
          }
          return error;
